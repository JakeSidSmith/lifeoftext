...
exports.leftPad = function (str, width, _char) {
  _char = _char || ' ';
  str = str.toString();
...
var find = function find(r, n) {
  ...
  if (r.has('#Value #Value')) {
    ...
    var _double = r.match('#Cardinal #Cardinal');

    if (_double.found && !r.has('(point|decimal)')) {
      //not 'two hundred'
      if (!_double.has('#Cardinal (#Multiple|point|decimal)')) {
        //one proper way, 'twenty one', or 'hundred one'
        if (!_double.has('(' + tens + ') #Cardinal') && !_double.has('#Multiple #Value')) {
          r.splitAfter(_double.terms(0).out('normal'));
        }
      }
    } //seventh fifth
...
var regexFn = function regexFn(t) {
  var _char = t.normal.charAt(t.normal.length - 1);

  if (regs[_char] === undefined) {
    return null;
  }
...
var splitMethods = function splitMethods(Terms) {
  var methods = {
    ...
    splitOn: function splitOn(reg, verbose) {
      ...
      all = all.map(function (ts) {
        return new Terms(ts, _this2.world, ts.refText, _this2.refTerms);
      });
      return all;
    },
...
var play = function (messages, input, newGame) {
    ...
    var advance = function (answer) {
        ...
        history.push(next);
        if (answer.prePrompt) {
            utils_1.displayLines(messages, answer.prePrompt, answer.color);
        }
    ...
    var matchAnswers = function (userInput) {
        ...
        for (var _i = 0, _a = lastPrompt.answers; _i < _a.length; _i++) {
            var answer = _a[_i];
...
var new_game_1 = require("./new-game");
var setup_1 = require("./setup");
var _a = setup_1.setup(), messages = _a.messages, input = _a.input;
...
Object.defineProperty(exports, "__esModule", { value: true });
var utils_1 = require("./utils");
var INTRODUCTION = 'Welcome! To play the game simply type what you would like to do, for example:\nI walk forward.\nI look around.\nI eat an apple.\n';
var introduction = function (messages) {
    utils_1.displayLines(messages, INTRODUCTION);
};
...
var displayLines = function (messages, text, className) {
    ...
    lines.forEach(function (line) {
        ...
        element.textContent = line ? line : constants_1.NBSP;
